...
The Python scope requires passing execution observations for both named property
artifacts. Selecting only the boundary test can pass pytest but fails `vt check`
because the translation property was not observed. Required keys omit revisions;
the report must still identify the exact imported revision.
...
The boundary property rejects the deliberate defect of replacing `>=` with `>`
in `expired`. Test that change in a disposable copy; failure must produce a
nonzero exit and the Python report must retain the corresponding `oft_id`.
...
disposable Git repository, checks correct behavior, injects the boundary defect,
